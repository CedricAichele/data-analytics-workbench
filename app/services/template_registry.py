...
TEMPLATES: dict[str, AnalyticsTemplate] = {
    ...
    "logistics": AnalyticsTemplate(
        ...
        status="Implemented",
        purpose="Analyze shipment lead time, on-time delivery, carriers, destinations, and shipping cost.",
        required_fields=LOGISTICS_REQUIRED_FIELDS,
        optional_fields=LOGISTICS_OPTIONAL_FIELDS,
        synonyms=LOGISTICS_SYNONYMS,
        sample_dataset=str(SAMPLE_LOGISTICS_PATH),
        implemented_page="app/pages/9_logistics_analytics.py",
        when_to_use="Use for shipments, delivery operations, and fulfillment performance datasets.",
        mapping_required=True,
        notes="Requires date fields for lead-time and delay calculations. Extra shipment attributes remain available in the working dataframe.",
    ),
    "finance": AnalyticsTemplate(
        template_id="finance",
        name="Finance Analytics",
        status="Implemented",
        purpose="Analyze revenue, cost, net result, margin, categories, cost centers, and budget variance.",
        required_fields=FINANCE_REQUIRED_FIELDS,
        optional_fields=FINANCE_OPTIONAL_FIELDS,
        synonyms=FINANCE_SYNONYMS,
        sample_dataset=str(SAMPLE_FINANCE_PATH),
        implemented_page="app/pages/10_finance_analytics.py",
        when_to_use="Use for finance ledgers, budget extracts, and transactional finance datasets.",
        mapping_required=True,
        notes="Requires an interpretable type column; the app does not guess revenue or cost meaning without it.",
    ),
}
